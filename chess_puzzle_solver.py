print("Welcome to the Chess Puzzle Solver !\n")
board_file = input("Enter the name of the file that has the piece and location information:\n")
opponent_file = input("Name of the file that contains moves you want to force your opponent into making:\n")


# Creates a 2D array containing chess piece information in the form of dictionaries
...
# Gets the movements the opponent is supposed to make in the form of tuples inside a list
...
# Gets the piece dictionary from a specific location in the board array
def get_piece(board, loc):
    return board[loc[0]][loc[1]]


# Formats chess location to array index format
def convert_to_index(location):
    return (8 - int(location[1]), ord(location[0]) - ord("a"))


# Formats location stored in tuples into chess location format
def convert_to_pos(location):
    return str(chr(location[1] + ord("a"))) + str(8 - location[0])


# Finds possible movements the pawn can make
...
# Finds possible movements the knight can make
...
# Finds possible movements the rook can make
...
# Finds possible movements the bishop can make
...
# Finds possible movements the queen can make
...
# Finds possible movements the king can make
...
# Finds all possible movements all pieces on the current board can make, creates and returns player - opponent move lists
...
# Moves a piece to a different location and updates its "location" key
...
# Check if there is a check in current board
...
# Find moves opponent can make that won't cause a check
...
# Check if two lists contain the same data
...
# Recursive function that tries to move pieces and prints the board when the correct move is found
    ...
    for move in player_moves_lst:
        ...
        if not true_op_moves:
            if not valid_opponent_moves:
                start, target = move
                print(f"Player makes the move: {convert_to_pos(start), convert_to_pos(target)}\n")
            ...
            print(f"Opponent makes the move: {convert_to_pos(start), convert_to_pos(target)}\n")
            board_printer(new_board)
...
# Find and print solution list
solution = solve(board, player_color, true_op_moves)
print("You should do the following moves:")
for i in solution:
    start, target = i
